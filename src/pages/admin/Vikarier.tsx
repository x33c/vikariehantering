...
export default function Vikarier() {
  ...
  if (laddar) return <div className="flex h-64 items-center justify-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-blue-600 border-t-transparent" /></div>;

  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-xl font-semibold text-gray-900">Vikarier</h1>
        <button onClick={() => setModal({ öppen: true })} className="rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700">
          + Lägg till vikarie
        </button>
      </div>
      ...
      {filtrerade.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-xl border-2 border-dashed border-gray-200 py-16">
          ...
          <button onClick={() => setModal({ öppen: true })} className="rounded-md bg-blue-600 px-3 py-1.5 text-xs font-medium text-white">Lägg till vikarie</button>
        </div>
      ) : (
              ...
              {filtrerade.map(v => (
                <tr key={v.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    {v.profil_id
                      ? <span className="rounded-full bg-green-100 px-2 py-0.5 text-xs font-medium text-green-700">Aktiv</span>
                      : <span className="rounded-full bg-gray-100 px-2 py-0.5 text-xs text-gray-500">Inget konto</span>}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex justify-end gap-1">
                      <button onClick={() => setModal({ öppen: true, rad: v })} className="rounded px-2 py-1 text-xs text-gray-600 hover:bg-gray-100">Redigera</button>
                      <button onClick={() => setRaderaId(v.id)} className="rounded px-2 py-1 text-xs text-gray-600 hover:bg-gray-100">Ta bort</button>
                    </div>
                  </td>
                </tr>
              ))}
